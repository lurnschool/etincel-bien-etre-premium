  ...
  return (
    <section className="relative paper-sand py-18 md:py-24 overflow-hidden">
      <Container>
        ...
        {/* ============================================================
            Composition cercle au sol — un seul layout responsive
            (Sprint K : Céline préfère voir le même schéma cercle sur
            mobile que sur desktop. On adapte les tailles en fluide
            plutôt que de basculer en grille.)

            Astuces responsive :
             - container max-w 340px mobile → 3xl desktop
             - radiusPct = 38 (compromis qui tient sur 340 et respire sur 3xl)
             - max-w label : 88px mobile → 190px desktop
             - tailles texte fluides
            ============================================================ */}
        <div className="relative max-w-[360px] sm:max-w-lg md:max-w-3xl mx-auto aspect-square md:aspect-[5/4] flex items-center justify-center">
          ...
          {/* Nœuds disposés autour — apparition stagger au scroll.
              radiusPct passe de 44 à 38 pour faire tenir le label sur
              mobile (340px de container) sans rogner. Tailles texte
              et max-w sont responsive. */}
          {nodes.map((node, i) => {
            const angle = (i / total) * Math.PI * 2 - Math.PI / 2;
            const radiusPct = 38;
            const left = 50 + Math.cos(angle) * radiusPct;
            const top = 50 + Math.sin(angle) * radiusPct;
            const startOffsetX = Math.cos(angle) * 6;
            const startOffsetY = Math.sin(angle) * 6;
            return (
              <motion.div
                key={node.label}
                ...
                className="absolute -translate-x-1/2 -translate-y-1/2 w-[88px] sm:w-[140px] md:w-[180px] text-center"
                style={{ left: `${left}%`, top: `${top}%` }}
              >
                <div className="inline-flex flex-col items-center">
                  <span
                    className="h-1 w-1 sm:h-1.5 sm:w-1.5 rounded-full bg-gold-deep mb-1 sm:mb-2 motion-safe:animate-[center-pulse_3s_ease-in-out_infinite]"
                    style={{ animationDelay: `${i * 0.3}s` }}
                  />
                  <p className="font-display-italic text-[0.7rem] sm:text-sm md:text-base text-gold-deep mb-0.5 sm:mb-1 leading-tight">
                    {node.label}
                  </p>
                  <p className="text-[0.55rem] sm:text-[0.7rem] md:text-xs leading-snug sm:leading-relaxed text-text-medium px-0.5">
                    {node.body}
                  </p>
                </div>
              </motion.div>
